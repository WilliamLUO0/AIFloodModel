...
class MaskWeightedL2Loss(nn.Module):
    """
    Masked weighted L2 regression loss (main loss).
    ...
    def __init__(
        ...
        if self.transform not in ("log1p", "asinh"):
            raise ValueError(f"[MaskWeightedL2Loss] transform must be log1p/asinh, got {self.transform}")
        if self.norm not in ("zscore", "minmax"):
            raise ValueError(f"[MaskWeightedL2Loss] norm must be zscore/minmax, got {self.norm}")
        ...
        if self.depth_mode not in ("exp", "linear", "power", "none"):
            raise ValueError(f"[MaskWeightedL2Loss] depth_mode must be exp/linear/power/none, got {self.depth_mode}")
        ...
        if stats_json == "":
            raise ValueError("[MaskWeightedL2Loss] stats_json is required.")
        if not os.path.exists(stats_json):
            raise FileNotFoundError(f"[MaskWeightedL2Loss] stats_json not found: {stats_json}")

        with open(stats_json, "r", encoding="utf-8") as f:
            meta = json.load(f)
        if "stats_var" not in meta:
            raise KeyError(f"[MaskWeightedL2Loss] stats_json must contain key 'stats_var': {stats_json}")
