...
def sync_token_dir(opt):
    d = osp.join(opt['path']['experiments_root'], '.sync_tokens')
    os.makedirs(d, exist_ok=True)
    return d


def sync_wait(token_path, timeout=14400, sleep_s=1.0, logger=None):
    start = time.time()
    while not osp.exists(token_path):
        if time.time() - start > timeout:
            raise RuntimeError(f'[SYNC] Wait timeout for token: {token_path}')
        time.sleep(sleep_s)
    if logger:
        logger.info(f'[SYNC] Detected token: {osp.basename(token_path)}')


def sync_signal(token_path, logger=None):
    with open(token_path, 'w') as f:
        f.write('')
    if logger:
        logger.info(f'[SYNC] Wrote token: {osp.basename(token_path)}')
...
def train_pipeline(root_path):
    ...
    if tb_logger and opt['rank'] == 0:
        logger.info(f"[TB] writer OK, logdir = {osp.join(opt['root_path'], 'tb_logger', opt['name'])}")
    else:
        logger.info("[TB] writer is None or not rank0")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['save_checkpoint_freq'] == 0:
                token_dir = sync_token_dir(opt)
                save_token = osp.join(token_dir, f'.save_done_{current_iter}')
                if opt['rank'] == 0:
                    if osp.exists(save_token):
                        try:
                            os.remove(save_token)
                        except Exception:
                            pass
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                token_dir = sync_token_dir(opt)
                val_token = osp.join(token_dir, f'.val_done_{current_iter}')
                if opt['rank'] == 0:
                    if osp.exists(val_token):
                        try:
                            os.remove(val_token)
                        except Exception:
                            pass
    ...
    token_dir = sync_token_dir(opt)
    final_save_token = osp.join(token_dir, '.final_save_done')
    if opt['rank'] == 0:
        if osp.exists(final_save_token):
            try:
                os.remove(final_save_token)
            except Exception:
                pass
    ...
    if opt.get('val') is not None:
        ...
        if opt['rank'] == 0:
            if osp.exists(final_val_token):
                try:
                    os.remove(final_val_token)
                except Exception:
                    pass
            for val_loader in val_loaders:
                model.validation(val_loader, current_iter, tb_logger, opt['val']['save_flood_map'])
