...
Changes vs FloodMapPFTV6 (see fmpftv6_arch.py):

1) Static feature downsampling channel ladder (`conv_static_f`)
   - V6 squeezed back to `static_in_chans` after every PixelUnshuffle:
       7 -> Unshuffle2 -> 28 -> Conv(28->7) -> 7 -> Unshuffle2 -> ...
     At scale=8 this discards ~75% of channel capacity per stage and
     ~98.5% over 3 stages, throwing away fine-grid high-frequency detail
     (DEM/roughness/twi/slope) before the deep feature module ever sees it.
   - V7 uses a geometric channel ladder that expands while spatial halves:
       7 -> Unshuffle2 -> 28 -> Conv(28->16) -> 16 -> Unshuffle2 -> 64 -> Conv(64->32) -> 32 -> Unshuffle2 -> 128 -> Conv(128->64) -> 64
     The final Conv lands directly on `num_feat` (64) so the extra trailing
     `Conv(static_in_ch -> num_feat)` from V6 is no longer needed.
   - The intermediate ladder is computed generically from (static_in_chans,
     num_feat, n_stages) so scale 2/4/8/16 all work without hardcoding.

2) Backward-incompatible state_dict
   - The Conv layer shapes inside `conv_static_f` change, AND the trailing
     `Conv(static_in_ch -> num_feat)` is removed, so V6 checkpoints cannot
     load into V7. Train from scratch.

Other modules (PFTB, attention, GroupNorm fusion, AOI gate, couple_mode,
upsample, depth/flood heads) are UNCHANGED from V6.
...
class FloodMapPFTV7(nn.Module):
    r""" FloodMapPFTV7
        A PyTorch impl of : `Progressive Focused Transformer for Single Image Super-Resolution`.

        Differences vs FloodMapPFTV6 (see fmpftv6_arch.py):
          - `conv_static_f`: geometric channel ladder instead of repeatedly
            squeezing back to `static_in_chans`. For scale=8 with
            `static_in_chans=7` and `num_feat=64`, the ladder is
            [7, 16, 32, 64]. The trailing `Conv(static_in_ch -> num_feat)`
            present in V6 is removed because the last PixelUnshuffle+Conv
            already lands on `num_feat`.
          - State_dict shapes inside `conv_static_f` change. Train from
            scratch; V6 checkpoints do NOT load into V7.
    ...
    def __init__(self,
        ...
        # V7: geometric channel ladder for static feature downsampling.
        # Each PixelUnshuffle(2) stage expands channels by 4 (lossless rearrange);
        # the following Conv lands on the next ladder value instead of squeezing
        # back to `static_in_chans`. The final stage directly outputs `num_feat`,
        # so no trailing `Conv(static_in_ch -> num_feat)` is needed.
        downs = []
        if self.upsampler == 'pixelshuffle':
            n_stages = int(math.log2(self.upscale))
            ladder = _build_static_ch_ladder(self.static_in_ch, num_feat, n_stages)
            self._static_ch_ladder = ladder
            for i in range(n_stages):
                downs.append(nn.PixelUnshuffle(2))
                in_c = ladder[i] * 4
                out_c = ladder[i + 1]
                downs.append(nn.Conv2d(in_c, out_c, 3, 1, 1))
                # V6 behavior preserved: every stage gets activation when enabled.
                if self.use_shallow_act:
                    downs.append(nn.LeakyReLU(negative_slope=0.2, inplace=True))
        elif self.upsampler == 'pixelshuffledirect':
            # Single-stage downsample: no intermediate bottleneck possible,
            # but still land directly on num_feat instead of routing through
            # `static_in_chans` like V6 did.
            self._static_ch_ladder = [self.static_in_ch, num_feat]
            downs.append(nn.PixelUnshuffle(self.upscale))
            downs.append(nn.Conv2d(self.static_in_ch * (self.upscale ** 2), num_feat, 3, 1, 1))
            if self.use_shallow_act:
                downs.append(nn.LeakyReLU(negative_slope=0.2, inplace=True))
        else:
            raise RuntimeError(f'[ERROR] yml.upsample ({self.upsampler}) needs to be "pixelshuffle" or '
                               f'"pixelshuffledirect"')
        self.conv_static_f = nn.Sequential(*downs)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = Upsample(upscale, num_feat, use_upsample_act=self.use_upsample_act)
    ...
    def forward(self, coarse_fm, static_f):
        ...
        coarse_fm = self.conv_coarse_fm(coarse_fm)  # [B, num_feat, Hc, Wc]
        coarse_fm = self.gn_coarse(coarse_fm)
        static_f = self.conv_static_f(static_f)    # [B, num_feat, Hc, Wc]
    ...
    def flops(self, input_resolution=None):
        ...
        # V7: static branch — use the geometric ladder.
        if self.upsampler == 'pixelshuffle':
            Hf, Wf = h * self.upscale, w * self.upscale
            ladder = self._static_ch_ladder
            curH, curW = Hf, Wf
            ...
            # NOTE: V6 had a trailing Conv(static_in_ch -> num_feat) here; V7
            # removes it because the last ladder step already lands on num_feat.
        else:
            # pixelshuffledirect: single Conv((static_in_ch * upscale^2) -> num_feat)
            flops += h * w * (self.static_in_ch * (self.upscale ** 2)) * num_feat * 9
        # conv_first
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            flops += h * w * self.embed_dim * num_feat * 9
            flops += self.upsample.flops(resolution)
            # conv last
            flops += (h * self.upscale) * (w * self.upscale) * num_feat * 1 * 9
            # flood head
            flops += (h * self.upscale) * (w * self.upscale) * num_feat * 3 * 1
        else:
            flops += self.upsample.flops(resolution)
            # flood head
            flops += (h * self.upscale) * (w * self.upscale) * 1 * 3 * 1
...
if __name__ == '__main__':
    ...
    print("static_ch_ladder:", model._static_ch_ladder)
