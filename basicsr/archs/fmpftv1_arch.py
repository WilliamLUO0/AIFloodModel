...
class FloodMapPFTV1(nn.Module):
    ...
    def forward(self, coarse_fm, static_f):
        # coarse-grid flood map: [B, 1, Hc, Hc], fine-grid static feature: [B, 7, Hf, Hf]
        B, _, Hc, Wc = coarse_fm.shape
        _, _, Hf, Wf = static_f.shape
        mod = self.window_size
        Hc_pad = ((Hc + mod - 1) // mod) * mod - Hc
        Wc_pad = ((Wc + mod - 1) // mod) * mod - Wc
        if (Hc_pad != 0) or (Wc_pad != 0):
            Hc, Wc = Hc + Hc_pad, Wc + Wc_pad
            coarse_fm = torch.cat([coarse_fm, torch.flip(coarse_fm, [2])], 2)[:, :, :Hc, :]
            coarse_fm = torch.cat([coarse_fm, torch.flip(coarse_fm, [3])], 3)[:, :, :, :Wc]

        if ((Hf / self.upscale) != Hc) or ((Wf / self.upscale) != Wc) or (Hc_pad != 0) or (Wc_pad != 0):
            raise RuntimeError(f'[ERROR] Hc{Hc}/Wc{Wc}, Hf{Hf}/Wf{Wf}, scale{self.upscale}')
