...
class BasicBlock(nn.Module):
        ...
        for i in range(self.depth):
            self.layers.append(
                PFTransformerLayer(
                    dim=self.dim,
                    block_id=self.idx,
                    layer_id=self.layer_id + i,
                    input_resolution=self.input_resolution,
                    num_heads=self.num_heads,
                    num_topk=self.num_topk,
                    window_size=self.window_size,
                    shift_size=0 if (i % 2 == 0) else self.window_size // 2,
                    convffn_kernel_size=self.convffn_kernel_size,
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, input_resolution={self.input_resolution}, depth={self.depth}, idx={self.idx}, layer_id={self.layer_id}, heads={self.num_heads}, window={self.window_size}, mlp_ratio={self.mlp_ratio}'
...
class PFTB(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            img_size=self.img_size, patch_size=self.patch_size, in_chans=0, embed_dim=self.dim, norm_layer=None)
        ...
        if resi_connection == '1conv':
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(self.dim, self.dim // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(self.dim // 4, self.dim // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
