...
Changes vs FloodMapPFTV5 (see fmpftv5_arch.py):

1) Shallow static feature downsampling (`conv_static_f`)
   - In V5, the `pixelshuffle` branch skipped the activation on the FIRST
     PixelUnshuffle + Conv stage (`if i > 0 and self.use_shallow_act`).
     That asymmetry was unprincipled: PixelUnshuffle is a pure rearrange,
     so the first stage is structurally identical to the rest.
   - V6 makes it uniform: when `use_shallow_act=True`, EVERY downsample
     stage gets a LeakyReLU. The flag still defaults to True.

2) Upsample module (only used by `pixelshuffle` upsampler)
   - V5's `Upsample` was `(Conv -> PixelShuffle) x log2(scale)` with no
     non-linearity between stages. V6 optionally inserts a LeakyReLU
     after each PixelShuffle EXCEPT the last one, controlled by a new
     `use_upsample_act` flag.
   - Position: `Conv -> PixelShuffle -> LeakyReLU` (post-shuffle, on
     num_feat channels). The final stage is intentionally left without
     activation so that the depth regression head (`conv_last`) receives
     features that can still represent negative label-space values
     (e.g. h=0 maps to ~-0.228 in asinh+zscore label space).
   - Default is `use_upsample_act=False` for backward compatibility with
     V5 ablation baselines.

3) Backward-incompatible state_dict
   - Adding LeakyReLUs adds no parameters, but the index of conv layers
     inside `nn.Sequential` shifts, so `state_dict` keys for `upsample.*`
     change when `use_upsample_act=True`. Resume from V5 checkpoints will
     require `strict=False` or explicit remapping.
...
class FloodMapPFTV6(nn.Module):
    r""" FloodMapPFTV6
        A PyTorch impl of : `Progressive Focused Transformer for Single Image Super-Resolution`.

        Differences vs FloodMapPFTV5 (see fmpftv5_arch.py):
          - `conv_static_f`: every downsample stage gets a LeakyReLU when
            `use_shallow_act=True` (V5 skipped the first stage).
          - `Upsample`: optional LeakyReLU after each PixelShuffle except
            the last stage, controlled by new `use_upsample_act` flag.
            Default False to keep parity with V5 ablation baselines.
    ...
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            for i in range(int(math.log2(self.upscale))):
                ...
                # V6: every stage gets activation (V5 skipped i=0).
