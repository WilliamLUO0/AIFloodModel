#!/bin/bash
#SBATCH --job-name=make_patches_ds16_ds4_ds2_h
...
# One entry per new downscaling factor: "scale:coarse_dx:coarse_minpatch:ds_tag".
# patch_fine = 64 * scale  ->  factor 2=128, 4=256, 16=1024.
# out-dir name: dataset_<ds_tag>_filtered_thr0p1_min<coarse_minpatch>100_full
#   (min<coarse><fine=100>, aligned with the existing dataset_ds8_..._min5100_full).
#
# RUNTIME: patch count ~ 1/patch_fine^2, so factor 2 (patch_fine=128) is ~16x the
# patches of factor 8, ~4x factor 4, ~64x factor 16. h-only estimates:
#   factor 16 ~1.5h, factor 4 ~6h, factor 2 ~23h  -> ~30h total (48h walltime).
# Order below is 16 -> 4 -> 2 (cheapest first) so a timeout only loses factor 2.
# To run a single factor, comment out the other lines.
FACTORS=(
  "16:dx128:2:ds16"
  "4:dx32:10:ds4"
  "2:dx16:25:ds2"
...
for spec in "${FACTORS[@]}"; do
  ...
  # h-only: the factor experiment trains water-depth models only. u/v patches
  # are NOT generated (would triple the runtime for models we won't run). If u/v
  # is ever needed for a factor, rerun that factor with `--var u v`.
  python tools/make_patches.py \
    ...
    --file-slope  "${BASE_INPUT}/Topo_Attrs/Slope_Deg.nc" \
    --file-twi  "${BASE_INPUT}/Topo_Attrs/TWI.nc" \
    --file-aspect-sin  "${BASE_INPUT}/Topo_Attrs/Aspect_SIN.nc" \
    --file-aspect-cos  "${BASE_INPUT}/Topo_Attrs/Aspect_COS.nc" \
...
# ============================================================
# PROVENANCE — the previous (factor 8) training-set build. Already done, kept
# verbatim for traceability. Coarse = dx64 min5, fine = dx8 min100, scale 8,
# patch_fine 512 -> dataset_ds8_filtered_thr0p1_min5100_full.
# (Launched from inside tools/, hence `python make_patches.py` with no prefix.)
# ============================================================
#
